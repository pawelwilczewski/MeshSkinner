...
struct DrawCallInfo
{
	...
	std::unordered_map<const Ref<Entity> &, const uint32_t> entities;
	// key: mesh already rendered, value: ibo index offset to use if mesh is reused
	std::unordered_map<const Ref<Mesh> &, const uint32_t> meshes;
...
private:
	static void SubmitMeshStatic(const Ref<Entity> &entity, const Ref<Mesh> &mesh, DrawCalls &drawCalls, std::function<void(VertexArray<uint32_t> &)> vaoInitFunction, std::function<uint32_t(VertexArray<uint32_t> &)> fillVertexBufferFunction);
